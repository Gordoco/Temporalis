...
public class CommandoAttack : AttackManager
{
    ...
    IEnumerator JetpackBoost(CharacterController controller, StatManager manager)
    {
        ...
        while (count < 50)
        {
            if (controller.enabled) controller.Move(Vector3.up * (float)manager.GetStat(NumericalStats.JumpHeight) * 0.001f);
            count++;
        ...
        Debug.Log(Vector3.up * (float)manager.GetStat(NumericalStats.JumpHeight) * 0.001f * 50 + " <- THIS HIGH EXPECTED, THIS HIGH ACTUAL -> " + (transform.position.y - start.y));
