...
public class ArmManager : NetworkBehaviour
{
    ...
    private IEnumerator TravelToAttack(GameObject enemy)
    {
        ...
        while (prog < 1)
        {
            ...
            prog += Time.deltaTime * (Vector3.Distance(initLocation, endLocation) * travelSpeed);
            if (enemy) endLocation = transform.InverseTransformPoint(enemy.transform.position);
